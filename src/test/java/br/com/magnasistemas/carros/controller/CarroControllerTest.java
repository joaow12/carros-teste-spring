...
public class CarroControllerTest {
	...
	void deveCadastrarUmCarro() {
		HttpEntity<CarroVO> requisicao = new HttpEntity<CarroVO>(new CarroVO("Golf", "987HJY", Marca.Volkswagen, "00000000011"));
		ResponseEntity<CarroVO> resposta = testRestTemplate.exchange("/carro/", HttpMethod.POST, requisicao, CarroVO.class);
	...
	void naoDeveCadastrarDoisCarrosComAMesmaPlaca() {
		...
		HttpEntity<CarroVO> requisicao = new HttpEntity<CarroVO>(new CarroVO("Golf", "000HHH", Marca.Volkswagen, "00000000011"));
		ResponseEntity<CarroVO> resposta = testRestTemplate.exchange("/carro/", HttpMethod.POST, requisicao, CarroVO.class);
	...
	void deveAtualizarUmCarro() {
		...
		if(c.isPresent()) {
			...
			ResponseEntity<CarroVO> resposta = testRestTemplate.exchange("/carro/{id}", HttpMethod.PUT, requisicao, CarroVO.class, c.get().getId());
	...
	void DeveDeletarUmCarroDoBanco() {
		...
		if(c.isPresent()) {
			...
			ResponseEntity<HttpStatus> resposta = testRestTemplate.exchange("/carro/{id}", HttpMethod.DELETE, null, HttpStatus.class, c.get().getId());
